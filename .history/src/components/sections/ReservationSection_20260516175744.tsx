...
export function ReservationSection() {
  ...
  const handleGroupWA = () => {
    window.open(
      `https://api.whatsapp.com/send?phone=${GROUP_WA}&text=${encodeURIComponent(
        `Halo kak! Mau tanya untuk Group / Corporate Kaizen All You Can Eat\n\nNama : \nTanggal Kedatangan : \nJumlah Pax : \nAsal Instansi : \nCabang Outlet yang Dituju : \nWaktu : 90 Menit / 120 Menit`
      )}`,
      "_blank"
    );
  };
  ...
  return (
    <section id="reservation" className="section-padding bg-kaizen-dark relative overflow-hidden">
      ...
      <style>{`
        ...
        .kaizen-input::placeholder { color: rgba(255,255,255,0.30); }
        .kaizen-input:hover { border-color: rgba(255,255,255,0.22); }
        .kaizen-input:focus { border-color: rgba(177,18,38,0.65); }
        .kaizen-input option { background-color: #1c1c1c; color: #ffffff; }
        .kaizen-input::-webkit-calendar-picker-indicator {
          filter: invert(1);
          opacity: 0.6;
          cursor: pointer;
        }
        .kaizen-select-wrap::after {
          content: '▾';
          position: absolute;
          right: 14px;
          top: 50%;
        ...
        <div className="grid lg:grid-cols-2 gap-10 lg:gap-16 items-start">
          ...
          >
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="relative kaizen-select-wrap">
              ...
              {/* WA Cabang indicator — muncul setelah pilih cabang */}
              {selectedBranch && (
                <motion.div
              ...
              <motion.button
                ...
                className="w-full mt-2 py-4 bg-[#25D366] hover:bg-[#1fb855] text-white font-bold text-sm tracking-[0.2em] uppercase flex items-center justify-center gap-3 transition-all duration-300 shadow-lg hover:shadow-xl hover:shadow-green-500/20 rounded-sm cursor-pointer"
              >
                ...
                {selectedBranch
                  ? `Reserve ke ${selectedBranch.name.replace("KAIZEN ", "")}`
                  : "Reserve via WhatsApp"}
              </motion.button>

              {/* Group Reservation button */}
              <button
                onClick={handleGroupWA}
          ...
          >
            <div>
              ...
              <div className="space-y-3">
                {Object.entries(BRANCHES).map(([key, branch]) => (
                    ...
                    className={`flex items-center justify-between px-4 py-3 border rounded-sm transition-all duration-300 group ${
                      form.location === key
                  ...
                  >
                    <div>
                      <p className={`text-xs font-semibold mb-0.5 transition-colors ${
                        form.location === key ? "text-green-400" : "text-white/60 group-hover:text-white"
                      }`}>
                        {branch.name.replace("KAIZEN ", "")}
                      </p>
                    ...
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"
                      className={`flex-shrink-0 transition-colors ${
                        form.location === key ? "text-green-400" : "text-white/20 group-hover:text-green-400"
                      }`}
                    >
                      <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
                    </svg>
            ...
            <div className="space-y-3">
                ...
                href={`https://api.whatsapp.com/send?phone=${GROUP_WA}&text=${encodeURIComponent("Halo kak! Mau tanya untuk Group / Corporate Kaizen All You Can Eat\n\nNama : \nTanggal Kedatangan : \nJumlah Pax : \nAsal Instansi : \nCabang Outlet yang Dituju : \nWaktu : 90 Menit / 120 Menit")}`}
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center gap-3 px-4 py-3 border border-kaizen-red/20 hover:border-kaizen-red/50 bg-kaizen-red/5 hover:bg-kaizen-red/10 rounded-sm transition-all duration-300 group"
              >
                ...
                href={`https://api.whatsapp.com/send?phone=${GROUP_WA}&text=${encodeURIComponent("Halo kak! Kami ingin mengajukan penawaran dengan data berikut\n\nNama : \nAsal Instansi : \nAlamat Instansi : \nPengajuan / Keperluan :\nCabang Outlet yang dituju : ")}`}
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center gap-3 px-4 py-3 border border-kaizen-gold/15 hover:border-kaizen-gold/40 bg-kaizen-gold/5 hover:bg-kaizen-gold/10 rounded-sm transition-all duration-300 group"
              >
